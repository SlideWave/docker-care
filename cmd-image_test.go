...
func TestFilterByAge(t *testing.T) {
	assert.Len(t, testImages.Filter(filterByAge(0)), 4)
	assert.Len(t, testImages.Filter(filterByAge(int(time.Hour*1/time.Second))), 3)
}

func TestFilter(t *testing.T) {
	assert.Len(t, testImages.
	Filter(filterByName("hoge")).
		Filter(filterByAge(int(time.Hour*3/time.Second))), 0)

	{
		image := testImages.
		Filter(filterByName("test")).
			Filter(filterByAge(int(time.Hour * 3 / time.Second)))
